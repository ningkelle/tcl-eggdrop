...
proc pub:currency {n u h c t} {
 ...
 set for_one [expr {double(round(100*$kursfrom))/100}]; set hasil [expr {round($kursfrom*$value)}]
 if {$value == "1"} {putnow "privmsg $c :[commify $value] [string toupper $from] \0034=\003 [commify $for_one] [string toupper $to]"} else {
  putnow "privmsg $c :[commify $value] [string toupper $from] \0034=\003 [commify $hasil] [string toupper $to] (1 [string toupper $from] \0034=\003 [commify $for_one] [string toupper $to])"
 }
}
