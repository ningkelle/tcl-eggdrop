...
proc pub:ggl {n u h c t} {
 ...
 if {$gimg == "image" || $gimg == "gambar" || $gimg == "img"} {set url "https://www.googleapis.com/customsearch/v1?key=$apikey&cx=$sEngine&searchType=image&lr=lang_id&safe=active&num=5&[http::formatQuery q $t]"} else {set url "https://www.googleapis.com/customsearch/v1?key=$apikey&cx=$sEngine&lr=lang_id&safe=active&num=5&[http::formatQuery q $t]"}
 if {[catch {set gglpage [http::geturl $url -timeout 60000]} error]} {putnow "privmsg $c :\0034ERROR:\003 $error"; return}
 set ggldata [http::data $gglpage]; set response [json::json2dict $ggldata]; http::cleanup $gglpage
 if {[dict exists $response error code]} {putnow "privmsg $c :\0034ERROR:\003 [dict get $response error message]"; return}
