...
proc pub:accuweather {n u h c t} {
 ...
 catch {exec curl -X GET --connect-timeout 5 https://dataservice.accuweather.com/locations/v1/cities/autocomplete?apikey=$apikey&language=id&q=$text} keydata
 ...
 regexp -all -nocase {\[\{\"Version.*?AdministrativeArea.*?\"ID\":\"(.*?)\",\"} $keydata "" StateID
 regexp -all -nocase {\[\{\"Version.*?AdministrativeArea.*?\"LocalizedName\":\"(.*?)\"\}\}} $keydata "" State
 ...
 set update [clock format $update -format "%d-%b-%Y %H:%M %Z" -timezone :Asia/Jakarta]; set temp [expr {round($temp)}]; set tempf [expr {round($tempf)}]; set visib [expr {round($visib)}]; set Wind [expr {double(round(10000*$Wind)/3600)/10}]; set pres [expr {round($pres)}]; set ceiling [expr {round($ceiling/1000)}]; set dewpoint [expr {round($dewpoint)}]
 set sunrise [clock format $sunrise -format "%H:%M %Z" -timezone :Asia/Jakarta]; set sunset [clock format $sunset -format "%H:%M %Z" -timezone :Asia/Jakarta]; if {$moonrise == "null"} {set moonrise "N/A"} else {set moonrise [clock format $moonrise -format "%H:%M %Z" -timezone :Asia/Jakarta]}; if {$moonset == "null"} {set moonset "N/A"} else {set moonset [clock format $moonset -format "%H:%M %Z" -timezone :Asia/Jakarta]}
 putnow "privmsg $c :\037Cuaca:\037 $LocalizedName, $StateID, $Country \0034-\003 $temp°C ($tempf°C) \0034-\003 [capitalize $WeatherText] \0034-\003 \037Awan:\037 $CloudCover\% \0034-\003 \037Ketinggian Awan:\037 $ceiling\km \0034-\003 \037Angin:\037 $Wind\m/s \0034-\003 \037Jarak Pandang:\037 $visib\km"
 putnow "privmsg $c :\037Kualitas Udara:\037 $airqualitytxt ($airquality) \0034-\003 \037UV Index:\037 $uvindextx \($uvindex\) \0034-\003 \037Tekanan:\037 $presten $pres\hPa \0034-\003 \037Kelembapan:\037 $RelativeHumidity\% \0034-\003 \037Titik Embun:\037 $dewpoint°C \0034-\003 \037Updated:\037 $update"
