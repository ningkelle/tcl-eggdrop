...
proc newline {input} {set j 0; set tempdef ""; foreach line [split $input \n] {if {$line != ""} {set len 350 ; set splitChr " "; set out [set cur {}]; set i 0; foreach word [split $line $splitChr] {if {[incr i [string len $word]]>$len} {lappend out [join $cur $splitChr]; set cur [list $word]; set i [string len $word]; incr j} else {lappend cur $word}; incr i}; lappend out [join $cur $splitChr]; foreach line2 $out {if {$j >= 1} {set line2 [linsert $line2 end]; set j [expr $j -1]}; lappend tempdef $line2}}}; return $tempdef}
