...
proc pub:ruptime {n u h c t} {
 ...
 foreach line $monitors {
  set fname [dict get $line friendly_name]
  set status [dict get $line status]
  if {$status == "0"} {set status "Paused"}
  if {$status == "1"} {set status "not checked yet"}
  if {$status == "2"} {set status "Up"}
  if {$status == "8"} {set status "seems down"}
  if {$status == "9"} {set status "Down"}
  set id [dict get $line id]
  set host [dict get $line url]
  set dtime [dict get $line create_datetime]
  set idtime [clock format $dtime -format "%d %b %Y %H:%M:%S %Z" -timezone :Asia/Jakarta]
  set uptime [dict get $line custom_uptime_ratio]
  set items "\n$fname ($id) \0034-\003 \037Status:\037 $status \0034-\003 \037Uptime:\037 $uptime\% \0034-\003 \037Created:\037 $idtime"
  append result $items
 }
